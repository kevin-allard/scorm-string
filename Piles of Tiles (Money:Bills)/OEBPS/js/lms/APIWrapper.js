...
function LMSInitialize()
{
   ...
   showDebugger("LMSInitialize: Result = "+initResult);
...
function LMSFinish()
{
   ...
   if (api == null) {
	  ...
	  showDebugger("LMSFinish: Result = "+result);
      var err = ErrorHandler();
   }
   return;
}
...
function LMSGetValue(name)
{
   ...
   if (api == null){
     // alert("Unable to locate the LMS's API Implementation.\nLMSGetValue was not successful.");
	  ...
	  showDebugger("LMSGetValue: Name = "+name+", Value = "+value);
//alert(name+":"+value);
      var err = ErrorHandler();
...
function LMSSetValue(name, value){
   ...
   if (api == null) {
     // alert("Unable to locate the LMS's API Implementation.\nLMSSetValue was not successful.");
	  ...
	  showDebugger("LMSSetValue: Name = "+name+", Value = "+value+", Result = "+result);
	  LMSCommit();
      var err = ErrorHandler();
   }
   return;
...
function LMSCommit() {
   ...
   if (api == null) {
	  ...
	  showDebugger("LMSCommit: Result = "+result);
      var err = ErrorHandler();
   }
   return;
}
...
function getAPI(){	
  ...
  while (tries < triesMax && theAPI == null){
    theAPI = findAPI(window);
    if (theAPI == null && window.parent != null) {
    	//alert("searching window.parent")
    	theAPI = findAPI(window.parent)
    }
    
    if (theAPI == null && window.top != null){
    ...
    if (theAPI == null && window.opener != null){
     	if (theAPI == null && window.opener.parent != null) {
    		//alert("searching window.opener.parent")
    	    	theAPI = findAPI(window.opener.parent)
    	    }
	   
	   
	   if (theAPI == null && window.opener.top != null){
		if (window.opener.top != null && !window.opener.top.closed){
			// alert("searching window.opener.top")
			theAPI = findAPI(window.opener.top);
		}
		
		if (theAPI == null && window.opener.top.opener != null && !window.opener.top.opener.closed){
			 //alert("searching window.opener.top.opener")
			 theAPI = findAPI(window.opener.top.opener);			
			
			if (theAPI == null && window.opener.top.opener.top != null && !window.opener.top.opener.top.closed){
				// alert("searching window.opener.top.opener.top")
				theAPI = findAPI(window.opener.top.opener.top);
			}			
		}	
	    }
    }
...
// Function created by Hurix for debugging purpose.
function showDebugger(str){
	//nwindow = window.open("","nw","width=600,height=600,scrollbars=yes")
	//nwindow.document.write(str+"<BR><BR>");
}
